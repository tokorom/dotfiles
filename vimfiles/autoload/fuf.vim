...
function fuf#enumExpandedDirsEntries(dir, exclude)
  ...
  " customize start
  if 0 < exists("g:fuf_file_recursiveLimit") && dirNormalized =~ '\*\*\/'
      let fileCount = system('find ./ -type f | wc -l')
      let fileCount = matchstr(fileCount, '\d\+')
      if fileCount > g:fuf_file_recursiveLimit
        let dirNormalized = substitute(dirNormalized, '**/', '', 'g')
      endif
  endif
  " customize finished
